...
class Favorite extends Component {
    ...
    loadFunc() {
        let items = this.state.items, count = this.state.count + 1;
        ...
        return(
            <div className={'content'}>
                <InfiniteScroll
                    ...
                    hasMore={this.state.items.length < this.state.favorite[this.state.favorite.length-1] ? true : false}
                    loader={<div className="loader" key={0}>Loading ...</div>}
                >
                    {this.state.items.map((item, key) => {
                        if (this.state.favorite.indexOf(key) !== -1) return(
